...
{
    public class ReactiveVariableReferencesEditorWindow : EditorWindow
    {
        private BaseReactiveVariableSO _reactiveVariableSO;
        private List<Component> _sceneReferenceComponents;
        private List<Object> _projectReferenceObjects;

        [MenuItem("Tools/ReactiveVariable References Searcher")]
        ...
        {
            if (Selection.activeObject is not BaseReactiveVariableSO)
            {
                return;
            }

            _reactiveVariableSO = Selection.activeObject as BaseReactiveVariableSO;

            DrawSearchReferenceInTheScene(_reactiveVariableSO);
        }
                ...
                foreach (var component in components)
                {
                    ...
                    while (serializedProperty.NextVisible(true))
                    {
                        if (serializedProperty.propertyType != SerializedPropertyType.ObjectReference ||
                            serializedProperty.objectReferenceValue != reactiveVariableSO)
                            continue;
